...
from core.schemas import layout_refiner_schema
from agents.agents import is_termination_msg, get_json_config, get_rationale_config
...
class JSONSchemaAgent(UserProxyAgent):
    ...
    def get_human_input(self, prompt: str) -> str:
        ...
        def extract_json_from_message(content: str):
            try:
                return parse_candidate(content)
                ...
                if match:
                    try:
                        return parse_candidate(match.group(1))
                    except (json.JSONDecodeError, ValueError, SyntaxError):
                        pass
                json_pattern = r'\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}'
                ...
                for candidate in matches:
                    try:
                        ...
                        continue
                raise ValueError(f"No valid JSON found in message: {content[:200]}...")

        json_obj_new = extract_json_from_message(message["content"])
...
def get_refiner_agents(model_name="gpt-4o", reasoning_effort="none"):
    ...
    layout_refiner = autogen.AssistantAgent(
        ...
        RESPONSE FORMAT:
        First, explain your reasoning for the spatial arrangement, considering object relationships and constraints.
